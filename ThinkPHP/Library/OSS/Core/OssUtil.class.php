...
class OssUtil
{
    ...
    public static function validateOptions($options)
    {
        ...
        if ($options != NULL && !is_array($options)) {
            exit($options . ':' . 'option must be array');
        }
    }
    ...
    {
        if (empty($content)) {
            exit("http body content is invalid");
        }
    }
    ...
    public static function generateFile($filename, $size)
    {
        if (file_exists($filename) && $size == filesize($filename)) {
            exit ($filename . " already exists, no need to create again. ");
            return;
        }
...
BBB;
        ...
        if ($fp) {
            while ($size > 0) {
                ...
                if (!$flag) {
                    exit("write to " . $filename . " failed. <br>");
                    break;
                }
            }
        } else {
            exit( "open " . $filename . " failed. <br>");
        }
        fclose($fp);
    }
    ...
    public static function decodeKey($key, $encoding)
    {
        ...
        if ($encoding == "url") {
            ...
            exit("Unrecognized encoding type: " . $encoding);
        }
    }
}
